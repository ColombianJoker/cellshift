...
def add_float_range_column(object, basecolumn, new_colname, minvalue=0, maxvalue=10, numranges=None, rangesize=1, decimals=1, onlystart=False):
  """
  Add range column to object, basing its values on basecolumn.
  Will round to n-decimals decimals
  """
  if isinstance(object, d.duckdb.DuckDBPyRelation) \
    and isinstance(basecolumn,str) \
    and isinstance(new_colname,str):
    ...
    table=object.to_table(table_name)
    if onlystart:
      add_col_str=f"ALTER TABLE {table_name} ADD COLUMN {new_colname} DECIMAL(18,{decimals})"
    else:
      add_col_str=f"ALTER TABLE {table_name} ADD COLUMN {new_colname} DECIMAL(18,{decimals})[]"
    d.sql(add_col_str)
    table=d.sql(f"FROM {table_name}")
    ...
    if numranges:
      ...
      if maxvalue:
        ...
        max_val_tuple=d.sql(max_val_str).fetchone()
        maxvalue=max_val_tuple[0]
      rangesize=(maxvalue-minvalue)/numranges
      if onlystart:
        upd_col_str=f"UPDATE {table_name} SET {new_colname}=FLOOR((FLOOR({basecolumn}/(({maxvalue}-{minvalue})/{numranges}))*(({maxvalue}-{minvalue})/{numranges}))*(10**{decimals}))/(10**{decimals})"
      else:
        rstart_str=f"FLOOR((FLOOR({basecolumn}/(({maxvalue}-{minvalue})/{numranges}))*(({maxvalue}-{minvalue})/{numranges}))*(10**{decimals}))/(10**{decimals})"
        rend_str=f"{rangesize}+FLOOR((FLOOR({basecolumn}/(({maxvalue}-{minvalue})/{numranges}))*(({maxvalue}-{minvalue})/{numranges}))*(10**{decimals}))/(10**{decimals})-(1/10**{decimals})"
        upd_col_str=f"UPDATE {table_name} SET {new_colname}=[{rstart_str},{rend_str}]"
    else:
      # The range size must be given
      numranges=ceil((maxvalue-minvalue)/rangesize)
      if onlystart:
        upd_col_str=f"UPDATE {table_name} SET {new_colname}=FLOOR((FLOOR({basecolumn}/(({maxvalue}-{minvalue})/{numranges}))*(({maxvalue}-{minvalue})/{numranges}))*(10**{decimals}))/(10**{decimals})"
      else:
        rstart_str=f"FLOOR((FLOOR({basecolumn}/(({maxvalue}-{minvalue})/{numranges}))*(({maxvalue}-{minvalue})/{numranges}))*(10**{decimals}))/(10**{decimals})"
...
def float_range_column(object, column, minvalue=0, maxvalue=0, numranges=None, rangesize=1, decimals=1, onlystart=False):
  """
  Replace all values in selected column with range values. column must be numeric, and number of ranges must be integer
  If onlystart==True, it replaces each value with the value at start of corresponding range.
  """
  if isinstance(object, d.duckdb.DuckDBPyRelation) \
    and isinstance(column,str):
